...
import { normalizeCallerRequest } from './filterNormalize';
...
export const requestCounts = (calls: CallReview[]) => anliegenOrder.map((key: AnliegenCategory) => ({ key, count: calls.filter(c => normalizeCallerRequest(c.anliegen) === key).length }));
